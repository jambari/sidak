<?php if(isset($options->model) && isset($options->type)): ?>
	...
	<?php if(class_exists($options->model)): ?>
		...
		<?php $relationshipField = @$options->column ?>
		...
		<?php if($options->type == 'belongsTo'): ?>
			...
			<?php else: ?>
				...
				<select class="form-control select2" name="<?php echo e($relationshipField); ?>">
					<?php 
						$model = app($options->model);
	            		$query = $model::all();
	            	?>
					<?php $__currentLoopData = $query; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $relationshipData): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
						<option value="<?php echo e($relationshipData->{$options->key}); ?>" <?php if($dataTypeContent->{$relationshipField} == $relationshipData->{$options->key}): ?><?php echo e('selected="selected"'); ?><?php endif; ?>><?php echo e($relationshipData->{$options->label}); ?></option>
		...
		<?php elseif($options->type == 'belongsToMany'): ?>
			...
			<?php else: ?>
				...
				<select class="form-control select2" name="<?php echo e($relationshipField); ?>[]" multiple>
